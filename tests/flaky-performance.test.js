...
describe("Flaky Performance and Resource Tests", () => {
  ...
  test("should track memory usage correctly (FLAKY: memory timing)", (done) => {
    ...
    const allocateMemory = (size) => {
      ...
      setTimeout(
        () => {
          const totalMemory = memoryAllocations.reduce(
  ...
  test("should load images correctly (FLAKY: image loading timing)", (done) => {
    ...
    const loadAllImages = async () => {
      await Promise.all([
        loadImage(
          img1,
          "data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMTAwIiBoZWlnaHQ9IjEwMCIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj48cmVjdCB3aWR0aD0iMTAwIiBoZWlnaHQ9IjEwMCIgZmlsbD0iYmx1ZSIvPjwvc3ZnPg==",
        ),
        loadImage(
          img2,
          "data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMTAwIiBoZWlnaHQ9IjEwMCIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj48cmVjdCB3aWR0aD0iMTAwIiBoZWlnaHQ9IjEwMCIgZmlsbD0icmVkIi8+PC9zdmc+",
        ),
        loadImage(
          img3,
          "data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMTAwIiBoZWlnaHQ9IjEwMCIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj48cmVjdCB3aWR0aD0iMTAwIiBoZWlnaHQ9IjEwMCIgZmlsbD0iZ3JlZW4iLz48L3N2Zz4=",
        ),
      ]);
    };
  ...
  // Stabilized TEST 5: Resource cleanup with deterministic timing
  test("should cleanup resources correctly (FLAKY: cleanup timing)", async () => {
    const contentArea = document.getElementById("content-area");
    ...
    expect(resourcesCreated).toBe(3);
    expect(resourcesCleaned).toBe(3);
    expect(cleanupCompleted).toBe(true);
    expect(contentArea.querySelectorAll(".resource")).toHaveLength(0);
  });
});
